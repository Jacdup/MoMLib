		    ...
		    {	
		        ...
		        if (pEdgeIndex + 1)
				{
					...
					for (ip = 0; ip < NumIntPoints; ip++)
					{
						...
						double r_dot_r_hat = (IntPoints[ip][0]*cos(r_hat[2]) + IntPoints[ip][1]*sin(r_hat[2]))*sin(r_hat[1]) + IntPoints[ip][2]*cos(r_hat[1]);
						...
						complex double temp_theta = (IntPoints[ip][3])*pRho_theta_comp*cexp(I*k*r_dot_r_hat);
						complex double temp_phi = (IntPoints[ip][3])*pRho_phi_comp*cexp(I*k*r_dot_r_hat);
						
						theta_holder = theta_holder + ((-I*w*mu*I_m*lp*temp_theta)/(4*M_PI*2*pArea));
						phi_holder = phi_holder + ((-I*w*mu*I_m*lp*temp_phi)/(4*M_PI*2*pArea));

...
%     candidate_dofs = dof_data.tri_to_dofs(interelem_VsrcZload(ii,1)).dofs; % select the first of the two tris arbitrarily, second would also work
    candidate_dofs = quad_blah(interelem_VsrcZload(ii,1),9:12);
% candidate_dofs = basis_supports_quad(interelem_VsrcZload(ii,1),:);
    for jj = 1:size(candidate_dofs,2)
        this_dof = candidate_dofs(1,jj);
          if sort(basis_supports_quad(this_dof,:),2) == sort(interelem_VsrcZload(ii,1:2),2) % this is the dof
...
%             ELength     = sqrt(edgevectemp*edgevectemp');
            ...
            if abs(V_val) > 1e-8
                if basis_supports_quad(this_dof,1) == interelem_VsrcZload(ii,1)
                    dofsign =  1;
                else
                    dofsign = -1;
                end

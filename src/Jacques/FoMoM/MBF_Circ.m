...
if endCap
    ...
    for i = length(triangle_blah)-endCapExclude+1:skip:length(triangle_blah)-numVertices% Every odd row, only first endcap
        row = row + 4;
...
%     col = col + 1;
    Rho(:,:,i+2) = [1,-1;1,1];
    for i = length(triangle_blah)-numVertices+skip:skip:length(triangle_blah)% Second endcap
        row = row + 4;

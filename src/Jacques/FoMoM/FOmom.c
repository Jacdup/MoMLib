	...
	for (i=0; i< T; i++) //Observation triangle
	{
       ...
       // mexPrintf("pPoints = %f %f %f\n", pPoints[0][0],pPoints[0][1], pPoints[0][2]);
        ...
        //----------------------------------------------------------------------------------------------------------------------------------
                        ...
                        }else
                        {
                            ...
                            {	
                                ...
                                if(d_36 > 0.375*max_diameter)
                                {
                                    ...
                                    //NumOuterIntPoints = 12;
                                    //NumInnerIntPoints = 12;
                                    count_12_12++;
                    ...
                    {	
                        //15/07/19 -> this is my problem, I assign the quadrature points before I even know if they will be used
                        //far interaction (further checks aren't required)
                        //mxMalloc outer integration points
                        //printf("further_check_required = %d",further_check_required);
...
//                         GaussianQuadrature(qPoints, NumInnerIntPoints, InnerIntPoints);
                    ...
                    }else
                    {
                        ...
                        //define pointer to inner integration points
        ...
        //----------------------------------------------------------------------------------------------------------------------------------
                ...
                //for (pvertex=0;pvertex<2;pvertex++){
                    ...
                    {	// This checks if the edge is a DOF and returns the edges index in the edge list
                       ...
                       // if(pEdgeIndex+1)
                        //{
                        //    //if its a valid basis function then use obs map for the index of the Z matrix 
                         //   //below is the final check to see if this basis function is an observer
                        //    pEdgeIndex = obs_map[(pTri[6+(ii+2)%3]-1)];
                       // }
                        //VERSION 3.2.2
                        int pEdgeIndex = pTri[6+(ii+2)%3];
                        int p_obs_index = obs_map[(pEdgeIndex-1)];
                        double ruv[3];
                        //int p_obs_index = obs_map[(pEdgeIndex-1 - pvertex)];
                        ...
                        {	
                           // mexPrintf("Edge = %d\n", pEdgeIndex);
                            ...
                            //I only want to assign the outer integration points here, meaning they will only be created once we know there will be maths?
...
//                                     if (isnan(creal(InnerIntPoints[0][1]))){
//                                         mexPrintf("pPoints = %f %f %f\n",pPoints[0][0], pPoints[0][1], pPoints[0][2]);
//                                         mexPrintf("pPoints = %f %f %f\n",pPoints[1][0], pPoints[1][1], pPoints[1][2]);
//                                         mexPrintf("pPoints = %f %f %f\n",pPoints[2][0], pPoints[2][1], pPoints[2][2]);
//                                         mexPrintf("%f %f %f\n", OuterIntPoints[oip][0],OuterIntPoints[oip][1],OuterIntPoints[oip][2]);
//                                         mexPrintf("%f %f %f\n", ruv[0],ruv[1],ruv[2]);
//                                         mexPrintf("qPoints = %f %f %f\n", qPoints[0][0], qPoints[0][1], qPoints[1][2]);
//                                     }
...
//                                         RAR_flag = 1;
                        ...
                        //----------------------------------------------------------------------------------------------------------------------------------	
                               ...
                               // MultiplyVectorByConstant(pRho, 3, pTri[3+(ii+2)%3]);
                                // 2020/01/16 Edit
                                // TODO: check edges. i.e. why (ii+2)? Correspond to BF function
                                double N_outer = BF(OuterIntPoints[oip], pPoints, (ii+2)%3, pTri[3+(ii+2)%3], pRho);
                                ParametricMap(pPoints, OuterIntPoints[oip], outerRuv);
                                
                                //mexPrintf("outer BF = %f %f %f\n", pRho[0],pRho[1],pRho[2]);


                                //for (qvertex=0;qvertex <2;qvertex++){
                                for (jj=0; jj<3; jj++) //Edges of testing triangle
                                { // Do this loop twice for other coefficient on edge
                                    
                                    int qEdgeIndex = qTri[6+(jj+2)%3];
                                    // This should be in total twice as big
                                    int q_src_index = src_map[(qEdgeIndex-1)]; 
                                    //int q_src_index = src_map[(qEdgeIndex-1-qvertex)]; 
                                   // int q_src_index_vertex2 = src_map[(qEdgeIndex-2)];
                                        ...
                                        for (iip=0; iip < newNumInt; iip++)
                                        {
                                           ...
                                           //MultiplyVectorByConstant( qRho, 3, qTri[3+(jj+2)%3]);
                                            ...
                                            if (RAR_flag){ // Then RAR1S was used
                                                ...
                                                if (isnan(Zeta[0]))
                                                {mexPrintf("error\n");}
                                                //Rm = Distance(outerRuv, InnerIntPoints[iip]);                                    
                                            }else{
                                            ...
                                            //if (abs(qTri[3+(jj+2)%3]) == 2){
                                            //    Phi = 0;}
                                          //  else {   
//                                             Phi += (qTri[3+(jj)%3]*N*2*temp)/(4*I*M_PI*w*eps);

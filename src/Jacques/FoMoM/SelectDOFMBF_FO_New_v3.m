...
if endCap == 1
    Rho(:,:,i+2) = [1,1;1,-1];
    for i = length(triangle_blah)-endCapExclude+2:2:length(triangle_blah)% Every odd row
        row = row + 4;
...
%     new = repmat(max_edge_vec_1,[numVertices 1]);
    theta1 = (acosd(dot(repmat(max_edge_vec_1,[numVertices 1]),edge_vecs(first_endcap,:),2)./(vecnorm(max_edge_vec_1,2,2)*vecnorm(edge_vecs(first_endcap,:),2,2))));
    theta2 = (acosd(dot(repmat(max_edge_vec_2,[numVertices 1]),edge_vecs(second_endcap,:),2)./(vecnorm(max_edge_vec_2,2,2)*vecnorm(edge_vecs(second_endcap,:),2,2))));

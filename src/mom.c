...
//                         GaussianQuadrature(qPoints, NumInnerIntPoints, InnerIntPoints);
        ...
        //								printf("\n%lf", Distance(qPoints[jj], qPoints[(jj+1)%3]));
                                    ...
                                    //{
                                        //if its a valid basis function then use obs map for the index of the Z matrix 
                                        //below is the final check to see if this basis function is a source
                                       // qEdgeIndex = src_map[(qTri[6+(jj+2)%3]-1)];
                                    //}
        ...
        // 									{
        // 									  temp = 3.0 + I*3.0;
        // 									  printf("why %f %f %d\n",creal(qPoints[(jj+2)%3][0]), creal(InnerIntPoints[iip][0]), newNumInt);
        // 									}
                                        ...
                                        complex double AdotpRho = A[0]*pRho[0] + A[1]*pRho[1] + A[2]*pRho[2];
                                                ...
                                                //printf("num_src = %d\n\n",num_src );
                                                //int Linear_I = (num_obs*(q_src_index-1)) + (p_obs_index-1);
                                                //printf("Linear I = %d p_obs_index = %d q_src_index = %d\n", Linear_I,p_obs_index ,q_src_index);
